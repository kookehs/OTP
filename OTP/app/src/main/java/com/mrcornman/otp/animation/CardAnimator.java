package com.mrcornman.otp.animation;
...
import com.mrcornman.otp.util.CardUtil;
...
public class CardAnimator{
    ...
    public void initLayout(){
        ...
        for(View v : mCardCollection){
            ...
            CardUtil.move(v, index * mStackMargin, 0);
            v.setRotation(0);
        }
    }
    ...
    private void setupRemotes(){
        ...
        mRemoteLayouts[0] = CardUtil.getMoveParams(topView, REMOTE_DISTANCE, -REMOTE_DISTANCE);
        mRemoteLayouts[1] = CardUtil.getMoveParams(topView, REMOTE_DISTANCE, REMOTE_DISTANCE);
        mRemoteLayouts[2] = CardUtil.getMoveParams(topView, -REMOTE_DISTANCE, -REMOTE_DISTANCE);
        mRemoteLayouts[3] = CardUtil.getMoveParams(topView, -REMOTE_DISTANCE, REMOTE_DISTANCE);
    ...
    public void drag(MotionEvent e1, MotionEvent e2, float distanceX,
        ...
        for(View v : mCardCollection){
            ...
            if(v!=getTopView() && index != 0){
                LayoutParams l = CardUtil.scaleFrom(v, mLayoutsMap.get(v), (int) (Math.abs(x_diff) * 0.05));
                CardUtil.moveFrom(v, l, 0, (int) (Math.abs(x_diff) * 0.1));
            }
        }
    }
